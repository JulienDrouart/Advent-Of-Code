...
{
    $action = "";
    if(strpos($instruction, "toggle") !== false){
        $action = "toggle";
        $instruction = str_replace($action,"",$instruction);
    }
    if(strpos($instruction, "turn on") !== false){
        $action = "turn on";
        str_replace($action,"",$instruction);
        $instruction = str_replace($action,"",$instruction);
    }
    if(strpos($instruction, "turn off") !== false){
        $action = "turn off";
        str_replace($action,"",$instruction);
        $instruction = str_replace($action,"",$instruction);
    }
    $instructionArray = explode(" ",$instruction);
    unset($instructionArray[0]);
    $instructionArray = array_values($instructionArray);
    $coo1 = explode(",",$instructionArray[0]);
    $coo2 = explode(",",$instructionArray[2]);

    foreach (range($coo1[1], $coo2[1]) as $cooY) { //on itère sur les Y
        foreach (range($coo1[0], $coo2[0]) as $cooX) { //on itère sur les Y
            switch ($action) {
                case "toggle":
                    if($grid[$cooY][$cooX] == 0)
                    {
...
print_r(array_count_values(array_values_recursive($grid))[1]);
...
foreach($array as $instruction)
{
    ...
    foreach (range($coo1[1], $coo2[1]) as $cooY) { //on itère sur les Y
        foreach (range($coo1[0], $coo2[0]) as $cooX) { //on itère sur les Y
            ...
            if($grid[$cooY][$cooX] < 0)$grid[$cooY][$cooX] =0;
        }
    }
}

print_r(array_sum(array_values_recursive($grid)));
